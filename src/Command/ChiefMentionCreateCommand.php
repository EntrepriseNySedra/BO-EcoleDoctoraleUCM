...
class ChiefMentionCreateCommand extends Command
{
    ...
    protected static $defaultName = 'app:chiefmention:create';

    protected function configure()
    {
        $this
            ->setDescription('Create a chief mention.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        try {
            ...
            $io->success(
                sprintf(
                    'Created chief mention with login=%s and password=%s', $input->getArgument('login'),
    ...
    private function createUser(InputInterface $input)
    {
        ...
        // check role
        $role = $this->roleManager->getRepository()->findOneByCode('ROLE_CHEFMENTION');
        if (!$role) {
            $role = $this->roleManager->create('Chef de mention', 'ROLE_CHEFMENTION');
        }

        // check profil
        $profil = $this->profilManager->findOneByRoleCode('ROLE_CHEFMENTION');
        if (!$profil) {
            $profil = $this->profilManager->create('Chef de mention', $role);
        }
