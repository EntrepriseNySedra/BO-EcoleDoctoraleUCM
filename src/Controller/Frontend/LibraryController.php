...
{    
     ...
     * List function
     * 
     * @Route("/bibliotheque/{slug}", name="library_article_list")
            ...
            ) : Response
    {
        ...
        $toArticleLibrary      = $articleRepository->findBy(array ('resourceUuid' => $rubrique->getUuid()));
        $toSubMenus = $menusService->getMenus();
        
        if(count($toArticleLibrary) === 1)
        {
            return $this->render(
                'frontend/library/details_article.html.twig',
                [
                    'article'       => $toArticleLibrary[0],
        ...
        return $this->render(
            'frontend/library/list.html.twig',
            [
                'rubrique'        => $rubrique,
                'derniereActus'   => $toActuList,
                'toArticle'       => $toArticleLibrary
            ]
        );
    }
     ...
     * Detail function
     * 
     * @Route("/bibliotheque/article/{slug}", name="library_details_article")
